...
from util import fix_sent_tokenization, legal_sent_tokenize, medical_sent_tokenize
...
def evaluate(
    ...
    **kwargs,
    ...
    elif training_domain == 'medical':
        model_kwargs['intrinsic_importance_model_id'] = 'cku41vkj_v0'
        model_kwargs['intrinsic_importance_model_url'] = 'https://www.dropbox.com/s/8birb30qhjxyqnr/model-cku41vkj%20v0.zip?dl=1'
    
    ##
    if dataset_name == 'legal':
        sent_tokenize_fn = legal_sent_tokenize
    elif dataset_name == 'medical':
        sent_tokenize_fn = medical_sent_tokenize
    ...
    if dataset_name == 'legal'  and content_type == 'oracle':
        ...
        data_path_ = Path(data_dir) / f"{split}.csv"
        par_data = pd.read_csv(data_path_)
        par_guidance = []
        for idx, row in par_data.iterrows():
            text = row['summary']
            if str(text) == '' or str(text) == 'nan':
                text = ''
            par_guidance.append(ROUGEContentGuidance(text))
